...
- **表現層（UI）**  
  提供使用者操作介面與資料呈現，包括 Web 介面、行動端介面及報表介面，方便不同角色使用。

- **應用層（Application）**  
  負責核心業務邏輯與流程控制，包含使用者管理模組、員工資料模組、出勤管理模組、薪資管理模組、報表與統計模組、通知模組，以及安全與日誌模組及 API / 外部系統接口。

- **資料層（Data Layer）**  
  負責資料儲存、存取與完整性維護，包括核心資料庫、日誌資料庫，以及備份與快照管理，確保資料安全與可回復性。
